...
LOG_LINEAR_DEBUG = bool(int(os.environ.get("LOG_LINEAR_DEBUG", "0")))
LOG_LINEAR_EPS = float(os.environ.get("LOG_LINEAR_EPS", "1e-9"))
LOG_LINEAR_MAX_EXP = float(os.environ.get("LOG_LINEAR_MAX_EXP", "40.0"))


def _ensure_finite(name: str, tensor: torch.Tensor, **context: any) -> None:
    if not LOG_LINEAR_DEBUG:
        return
    with torch.no_grad():
        if tensor is None:
            return
        if torch.isfinite(tensor).all():
            return
        coords = torch.nonzero(~torch.isfinite(tensor))
        info = {k: v for k, v in context.items()}
        info_str = ", ".join(f"{k}={v}" for k, v in info.items()) if info else "no-context"
        raise RuntimeError(
            f"[LogLinearDebug] Non-finite values detected in {name} "
            f"(shape={tuple(tensor.shape)}, {info_str}); sample index={coords[0].tolist()}"
        )
...
def chunkwise_fwd_kernel(
    ...
    EPS: tl.constexpr = LOG_LINEAR_EPS,
    MAX_EXP: tl.constexpr = LOG_LINEAR_MAX_EXP,
):
    ...
    for i_t in range(NT):
        ...
        valid_rows = (i_t * BT + i_idx) < T
        b_h_mask = (i_idx >= j_idx) & valid_rows
        b_h = tl.load(b_h_ptrs, mask=b_h_mask, other=0.0)
        ...
        if MAX_EXP > 0:
            b_g = tl.maximum(tl.minimum(b_g, MAX_EXP), -MAX_EXP)
        exp_b_g = tl.exp(b_g)
        if EPS > 0:
            exp_b_g = tl.maximum(exp_b_g, EPS)
        ...
        if MAX_EXP > 0:
            diff = tl.maximum(tl.minimum(diff, MAX_EXP), -MAX_EXP)
        exp_diff = tl.exp(diff)
        if EPS > 0:
            exp_diff = tl.maximum(exp_diff, EPS)
        b_s = (tl.dot(b_q, b_k) * tl.where((i_idx >= j_idx) & m_t[:, None] & m_t[None, :], exp_diff, 0)).to(
            b_q.dtype,
        ) * b_h
        ...
        if MIN_LEVEL <= 0 and MAX_LEVEL >= 0:
            if chunk_index & 1:
                ...
                b_o += tl.dot((b_l * b_q), kv_0.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 1 and MAX_LEVEL >= 1:
            if chunk_index & 2:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_1.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 2 and MAX_LEVEL >= 2:
            if chunk_index & 4:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_2.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 3 and MAX_LEVEL >= 3:
            if chunk_index & 8:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_3.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 4 and MAX_LEVEL >= 4:
            if chunk_index & 16:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_4.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 5 and MAX_LEVEL >= 5:
            if chunk_index & 32:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_5.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 6 and MAX_LEVEL >= 6:
            if chunk_index & 64:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_6.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 7 and MAX_LEVEL >= 7:
            if chunk_index & 128:  # 8192 - 16384
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_7.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 8 and MAX_LEVEL >= 8:
            if chunk_index & 256:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_8.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 9 and MAX_LEVEL >= 9:
            if chunk_index & 512:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_9.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 10 and MAX_LEVEL >= 10:
            if chunk_index & 1024:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_10.to(b_q.dtype)) * exp_b_g[:, None]
        if MIN_LEVEL <= 11 and MAX_LEVEL >= 11:
            if chunk_index & 2048:
                ...
                b_l = tl.load(p_l, boundary_check=(0, 1))
                b_o += tl.dot((b_l * b_q), kv_11.to(b_q.dtype)) * exp_b_g[:, None]
        ...
        if i_t < NT - 1 or T % BT == 0:
            ...
            if MAX_EXP > 0:
                b_g_last = tl.maximum(tl.minimum(b_g_last, MAX_EXP), -MAX_EXP)
            exp_b_g_last = tl.exp(b_g_last)
            if EPS > 0:
                exp_b_g_last = tl.maximum(exp_b_g_last, EPS)
            if KV_0_CREATED:
                kv_0 *= exp_b_g_last
            if KV_1_CREATED:
                kv_1 *= exp_b_g_last
            ...
            diff_bg = b_g_last - b_g
            if MAX_EXP > 0:
                diff_bg = tl.maximum(tl.minimum(diff_bg, MAX_EXP), -MAX_EXP)
            exp_diff_bg = tl.exp(diff_bg)
            if EPS > 0:
                exp_diff_bg = tl.maximum(exp_diff_bg, EPS)
            b_v = (b_v * exp_diff_bg[:, None]).to(b_v.dtype)
            if MIN_LEVEL <= 1:
                kv_0 += tl.dot(b_k, b_v)
            elif MIN_LEVEL == 2:
                kv_1 += tl.dot(b_k, b_v)
...
def chunkwise_bwd_kernel_dhg(
    ...
    for i_t in range(tl.cdiv(T, BT) - 1, -1, -1):
        ...
        if i_t & (1 << ell):  # compute this chunk
            ...
            if EPS > 0:
                exp_b_g = tl.maximum(exp_b_g, EPS)
            b_q = (b_q * (exp_b_g * b_l)[None, :]).to(b_q.dtype)
...
def chunkwise_bwd_kernel_hdqgl(
    ...
    for i_t in range(tl.cdiv(T, BT)):
        ...
        if i_t & (1 << ell):  # compute and store derivatives
            ...
            if EPS > 0:
                exp_b_g = tl.maximum(exp_b_g, EPS)
            b_dlq = exp_b_g[:, None] * tl.dot(b_do, b_h.to(b_do.dtype))
        ...
        if (i_t & (1 << ell)) == 0:  # update the state
            ...
            if EPS > 0:
                exp_diff_bg = tl.maximum(exp_diff_bg, EPS)
            b_k = (b_k * exp_diff_bg[:, None]).to(b_k.dtype)
            b_h += tl.dot(b_v, b_k)
...
def chunkwise_bwd_kernel_dkg(
    ...
    if EPS > 0:
        exp_diff_bg = tl.maximum(exp_diff_bg, EPS)
    b_v32 = b_v.to(tl.float32)
    b_dh32 = b_dh.to(tl.float32)
    dot_v_dh = tl.dot(b_v32, b_dh32)
    b_dk = tl.where(m_t, exp_diff_bg, 0)[:, None] * dot_v_dh
    b_dg = tl.load(p_dg, boundary_check=(0,))
    b_k32 = b_k.to(tl.float32)
    b_dg -= tl.sum(b_k32 * b_dk, axis=1)
    b_dg_last += tl.sum(b_dk * b_k32)
...
def chunkwise_bwd_kernel_dv(
    ...
    b_k32 = b_k.to(tl.float32)
    b_dh32 = b_dh.to(tl.float32)
    dot_k_dh = tl.dot(b_k32, b_dh32)
    b_dv = tl.where(m_t, exp_diff_bg, 0)[:, None] * dot_k_dh
...
def chunkwise_bwd_kernel_diag(
    ...
    m_t = i_t * BT + o_i < T
    diff_bg = b_g[:, None] - b_g[None, :]
    if MAX_EXP > 0:
        diff_bg = tl.maximum(tl.minimum(diff_bg, MAX_EXP), -MAX_EXP)
    exp_diff_bg = tl.exp(diff_bg)
    if EPS > 0:
        exp_diff_bg = tl.maximum(exp_diff_bg, EPS)
    b_a = tl.where((i_idx >= j_idx) & m_t[:, None] & m_t[None, :], exp_diff_bg, 0)
...
class ChunkLogLinearAttentionFunction(torch.autograd.Function):
    ...
    def backward(ctx, do, dht):
        ...
        dh = torch.zeros((B, NT, H, K, V), dtype=torch.float, device=v.device)
        ...
        _ensure_finite("log_linear_backward_dq", dq, stage="dq")
        _ensure_finite("log_linear_backward_dk", dk, stage="dk")
        _ensure_finite("log_linear_backward_dv", dv, stage="dv")
        _ensure_finite("log_linear_backward_dg", dg, stage="dg")
        _ensure_finite("log_linear_backward_dl", dl, stage="dl")
...
def chunk_log_linear_attn(
    ...
    _ensure_finite("log_linear_attn_chunk", o, stage="chunk_output")
    if final_state is not None and final_state.ht is not None:
        _ensure_finite("log_linear_attn_chunk_state", final_state.ht, stage="chunk_state")
