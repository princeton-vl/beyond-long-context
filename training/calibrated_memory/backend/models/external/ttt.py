...
if is_causal_conv1d_available():
    try:
        ...
        # Some cluster envs expose the package metadata but ship a wheel built
        # against an incompatible Torch/CUDA ABI. Treat that the same as the
        # extension being unavailable so the pure PyTorch path still works.
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        should_prime_from_cache = (
            last_mini_batch_params_dict is None
            and cache_params is not None
            and getattr(cache_params, "seqlen_offset", 0) > 0
        )
        if should_prime_from_cache:
