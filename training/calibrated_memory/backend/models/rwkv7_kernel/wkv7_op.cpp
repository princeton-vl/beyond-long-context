...
#define WKV7_SUFFIX "_" STR(_C_) "_" STR(_CHUNK_LEN_)
#define WKV7_FORWARD_OP "forward" WKV7_SUFFIX
#define WKV7_BACKWARD_OP "backward" WKV7_SUFFIX
...
TORCH_LIBRARY(wind_backstepping, m) {
    m.def(WKV7_FORWARD_OP "(Tensor w, Tensor q, Tensor k, Tensor v, Tensor z, Tensor a, Tensor(a!) y, Tensor(b!) s, Tensor(c!) sa) -> ()");
    m.def(WKV7_BACKWARD_OP "(Tensor w, Tensor q, Tensor k, Tensor v, Tensor z, Tensor a, Tensor dy, Tensor s, Tensor sa, Tensor(a!) dw, Tensor(b!) dq, Tensor(c!) dk, Tensor(d!) dv, Tensor(e!) dz, Tensor(f!) da) -> ()");
}

TORCH_LIBRARY_IMPL(wind_backstepping, CUDA, m) {
    m.impl(WKV7_FORWARD_OP, &forward);
    m.impl(WKV7_BACKWARD_OP, &backward);
}
